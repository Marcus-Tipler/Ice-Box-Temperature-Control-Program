...
timer = ((currentTime - StartTime)/1000); // Déclare la variable de "timer" qui vaut le temps actuelle - le temps de depart diviser par mille.
  ...
  energycons = (CalcEmoy + energy); // Calcule l'énergie totale consommé du système en Wh.
...
void MesureDegC(){
  ...
  Serial.println("Temperature = " + String(Temperature) + " DegC"); // Impression de la valeur de température en Degrées Celcius sur le Serial Moniteur.
  ...
  valeurAdcConsigne = analogRead(tempCons); // Déclarer la valeur consigne ADC avec la lecture analogique de "tempCons".
  Consigne = valeurAdcConsigne/35.0; // Déclarer la consigne avec la valeur de consigne ADC et une fonctione mathématiques.
